...
if (-not $SkipBackup) {
    ...
    ssh -i $SSH_KEY $SSH_HOST "sudo cp $REMOTE_PATH/server/.env /tmp/.env.backup 2>/dev/null; echo 'Backup OK'"
}

Write-Host "[2/5] Copie des fichiers (index.html + server)..." -ForegroundColor Yellow
scp -i $SSH_KEY -r "$LOCAL_PATH\index.html" "$SSH_HOST`:$REMOTE_PATH/index.html"
scp -i $SSH_KEY -r "$LOCAL_PATH\server\server.js" "$SSH_HOST`:$REMOTE_PATH/server/server.js"
scp -i $SSH_KEY -r "$LOCAL_PATH\server\package.json" "$SSH_HOST`:$REMOTE_PATH/server/package.json"
...
if (-not $SkipNpmInstall) {
    ...
    ssh -i $SSH_KEY $SSH_HOST "cd $REMOTE_PATH/server && npm install 2>&1 | tail -1"
}
